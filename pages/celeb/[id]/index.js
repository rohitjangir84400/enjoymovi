...
export async function getStaticProps(context) {
  ...
  const res = await axios(`https://api.themoviedb.org/3/person/${id}?api_key=${process.env.API_KEY}&language=en-US&page=1`)
  const latestcelebs = res.data;
  const resp = await axios(`https://api.themoviedb.org/3/person/${id}/images?api_key=${process.env.API_KEY}&language=en-US&page=1`)
