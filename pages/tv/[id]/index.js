...
export default function Populartv({ poptv, tvvideo }) {
  ...
  return (
    <>
      <Meta title={poptv.name} />
      <div className="" style={{
        backgroundImage: `url('https://image.tmdb.org/t/p/original${poptv.backdrop_path}')`, backgroundPosition: "center", backgroundSize: "cover",
      }} >
        <div className="bg-[hsl(207,19%,11%)]/[0.9] py-[100px] px-5">
          ...
          <div className="h-[100%] flex items-center gap-10">
              ...
              <div className="text-[14px] h-[100%] p-2 hover:cursor-pointer">
                <div className="flex flex-col justify-center">
                  <div className="overflow-hidden aspect-[2/3] relative mb-2" >
                    <Image className=" rounded-md" src={`https://image.tmdb.org/t/p/original${poptv.poster_path}`} alt="failed to load" width='100%' height='100%'
                      layout="fill" objectFit="cover" />
                  </div>
                  <div className="text-white">
                    <div className="flex justify-between items-center gap-[5px] mb-3">
                      <div className="w-[60%] font-bold hover:text-[#e2d703]">{poptv.name}</div>
                      <div className="flex items-center gap-2"><CalendarDaysIcon className="h-5 w-5 fill-[#e2d703]" />{poptv.last_air_date}</div>
                    </div>
                    <div className="flex justify-between items-center mb-2">
                      <div className="border-2 py-1 px-2 text-[#e2d703]">
                        {poptv.original_language}
                      </div>
                      <div className="flex justify-between items-center gap-5">
                        <div className="flex items-center gap-2  text-[14px]"><ClockIcon className="h-5 w-5 fill-[#e2d703]" />{poptv.number_of_episodes}</div>
                        <div className="flex items-center gap-2"><StarIcon className="h-5 w-5 fill-[#e2d703]" />{poptv.number_of_seasons}</div>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="basis-[9/12]">
              <div className="text-[46px] text-[#e2d703] font-bold">{poptv.original_name}</div>
              <div className="text-[12px] text-white border border-[#e2d703] rounded-lg p-5">{poptv.overview}</div>
              <div className="border border-[#e2d703] w-fit py-3 px-1 rounded-lg text-white mt-2"><span className="text-[#e2d703]">Seasons:</span> {poptv.number_of_seasons}</div>
              <div className="border border-[#e3d703] px-2 py-3 rounded-lg mt-2 w-fit text-white"><span className="text-[#e2d703]">Episodes: </span> {poptv.number_of_episodes}</div>
              <div className="text-white mt-2">Production Companies</div>
              <div className="flex flex-wrap gap-2 mt-2">{
                poptv.production_companies.map(com => {
                  return (
